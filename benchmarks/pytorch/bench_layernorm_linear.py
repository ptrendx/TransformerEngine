...
"""Benchmarks for te.LayerNormLinear module (fused LayerNorm + Linear)."""
...
def test_bench_layernorm_linear(
    ...
    layer = te.LayerNormLinear(k, n, bias=False, params_dtype=torch.bfloat16).cuda()
    x = torch.randn(
    ...
    result = benchmark_timer.measure(
        ...
        label="te.LayerNormLinear",
        sub_label=f"{m}x{k}x{n}_{recipe_name}_{direction}",
        name="te.LayerNormLinear",
        category="layernorm_linear",
