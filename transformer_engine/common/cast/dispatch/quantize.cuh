...
void group_quantize_fwd_helper(const NVTEGroupedTensor input, NVTEGroupedTensor output,
  ...
  switch (scaling_mode) {
    case NVTE_DELAYED_TENSOR_SCALING: {
      if (is_fp8_dtype(output_tensor->dtype()) && output_tensor->has_columnwise_data()) {
        NVTE_ERROR(fp8::kGroupedFP8TensorScalingColumnwiseUnsupported);
      }
      if constexpr (IS_ACT) {
        NVTE_ERROR(fp8::kGroupedFP8TensorScalingFusedActivationUnsupported);
      } else {
        fp8::group_quantize(input_tensor, noop_tensor, output_tensor, &quant_config_cpp, stream);
      }
      break;
    }
    case NVTE_MXFP8_1D_SCALING: {
...
void group_quantize_bwd_helper(const NVTEGroupedTensor grad, const NVTEGroupedTensor input,
  ...
  switch (scaling_mode) {
    case NVTE_DELAYED_TENSOR_SCALING: {
      ...
      NVTE_ERROR(fp8::kGroupedFP8TensorScalingFusedBackwardUnsupported);
      break;
    }
    case NVTE_MXFP8_1D_SCALING: {
