...
void group_quantize_bwd_helper(const NVTEGroupedTensor grad, const NVTEGroupedTensor input,
  ...
  switch (scaling_mode) {
    case NVTE_DELAYED_TENSOR_SCALING: {
      NVTE_CHECK(!IS_DBIAS && !IS_DACT,
                 "Grouped FP8 tensor-scaling fused dbias/dactivation quantize is not supported.");
      fp8::group_quantize</*IS_ACT=*/false, ParamOP, OP>(grad_tensor, noop_tensor, output_tensor,
                                                         &quant_config_cpp, stream);
      break;
    }
    case NVTE_MXFP8_1D_SCALING: {
