...
__device__ __forceinline__ uint32_t load_global_cs_u32(const uint32_t* ptr) {
  uint32_t value;
  asm volatile("ld.global.cs.b32 %0, [%1];" : "=r"(value) : "l"(ptr) : "memory");
  return value;
}
...
__host__ __device__ constexpr int row_coalesced_smem_physical_row(const int row) {
...
__device__ void swizzle_row_scaling_coalesced_full_m_impl(
    const void* input, void* output, const int K, const int first_m_tile, int4* slm_v4i) {
  constexpr int SF_TILE_SIZE = SF_TILE_DIM_M * SF_TILE_DIM_K;
  constexpr int TILE_OUTPUT_V4 = SF_TILE_SIZE / static_cast<int>(sizeof(int4));
  const int row_v4 = K / static_cast<int>(sizeof(int4));
  const int tile_v4 = SF_TILE_DIM_M * row_v4;
...
#pragma unroll
    ...
    if (tile_v4 <= blockDim.x && M_TILES_PER_BLOCK > 1) {
      int4 values[M_TILES_PER_BLOCK];
      const int row = rows[0];
      const int col_i32 = col_i32s[0];
      const int tile_idx = tile_idx_base;
      const int smem_base = row_coalesced_smem_physical_row(row) * smem_stride_i32 + col_i32;
...
#pragma unroll
      for (int local_m_tile = 0; local_m_tile < M_TILES_PER_BLOCK; ++local_m_tile) {
        const int global_m = (first_m_tile + local_m_tile) * SF_TILE_DIM_M;
        const size_t input_tile_v4 = static_cast<size_t>(global_m) * row_v4;
        values[local_m_tile] = load_global_cs(input_v4 + input_tile_v4 + tile_idx);
      }
...
#pragma unroll
        for (int prefetch = 0; prefetch < ROW_COALESCED_LOAD_PREFETCH; ++prefetch) {
          if (valid[prefetch]) {
            const int tile_idx = tile_idx_base + prefetch * blockDim.x;
            values[prefetch] = load_global_cs(input_v4 + input_tile_v4 + tile_idx);
          }
        }
...
template <int SF_TILE_DIM_M, int SF_TILE_DIM_K, int M_TILES_PER_BLOCK>
__global__ void __launch_bounds__(ROW_COALESCED_THREADS)
    swizzle_row_scaling_coalesced_full_m_kernel(const void* input, void* output, const int K) {
  extern __shared__ int4 slm_v4i[];
  const int first_m_tile = blockIdx.x * M_TILES_PER_BLOCK;
...
__device__ __forceinline__ uint4 load_col_swizzle_segment_full_m(const uint8_t* input, const int M,
                                                                 const int m_base, const int k,
                                                                 const int segment) {
  const int m = m_base + segment * static_cast<int>(sizeof(uint4));
  return load_global_cs(reinterpret_cast<const uint4*>(input + static_cast<size_t>(k) * M + m));
}
...
__device__ __forceinline__ uint32_t load_col_swizzle_shared_byte_from_word(
    const uint32_t* tile_u32, const int row_stride_bytes, const int k, const int row,
    const int m_group) {
  const int row_word = row & ~3;
  const int byte_shift = (row & 3) * 8;
  const int byte_offset = k * row_stride_bytes + row_word + m_group * 32;
  const uint32_t packed = tile_u32[byte_offset / static_cast<int>(sizeof(uint32_t))];
...
__device__ void swizzle_col_scaling_coalesced_tile_impl(
  ...
  for (int idx = threadIdx.x; idx < active_k * M_TILE_V4; idx += blockDim.x) {
    ...
    if constexpr (FULL_TILE) {
      value = load_col_swizzle_segment_full_m(input_u8, M, m_base, k_base + k_rel, segment);
    } else {
      value = load_col_swizzle_segment(input_u8, M, original_M, original_K, m_base,
                                       k_base + k_rel, segment);
    }
  ...
  const uint32_t* slm_u32_read = reinterpret_cast<const uint32_t*>(slm_i32);
...
#pragma unroll
      for (int k_group = 0; k_group < SF_TILE_DIM_K; ++k_group) {
        word |= load_col_swizzle_shared_byte_from_word(
                    slm_u32_read, SMEM_STRIDE_BYTES, k_rel_base + k_group, row_in_new_tile,
                    m_group)
                << (8 * k_group);
      }
...
template <int SF_TILE_DIM_M, int SF_TILE_DIM_K, bool FULL_TILE = false>
__device__ void swizzle_col_scaling_coalesced_batched_m_impl(
    const void* input, void* output, const int M, const int K, const int original_M,
    const int original_K, const int m_tile_block, const int k_tile_block,
    const int k_tiles_per_block, const int m_tiles_per_block, int* slm_i32) {
  const int num_tiles_m = M / SF_TILE_DIM_M;
  const int first_m_tile = m_tile_block * m_tiles_per_block;
  const int remaining_m_tiles = num_tiles_m - first_m_tile;
  const int active_m_tiles =
      remaining_m_tiles <= 0 ? 0
                             : (remaining_m_tiles < m_tiles_per_block ? remaining_m_tiles
                                                                       : m_tiles_per_block);
  ...
  for (int local_m_tile = 0; local_m_tile < active_m_tiles; ++local_m_tile) {
    swizzle_col_scaling_coalesced_tile_impl<SF_TILE_DIM_M, SF_TILE_DIM_K, FULL_TILE>(
        input, output, M, K, original_M, original_K, first_m_tile + local_m_tile, k_tile_block,
        k_tiles_per_block, slm_i32);
    if (local_m_tile + 1 < active_m_tiles) {
      __syncthreads();
    }
  }
}
...
__global__ void __launch_bounds__(COL_COALESCED_THREADS)
  ...
  swizzle_col_scaling_coalesced_batched_m_impl<SF_TILE_DIM_M, SF_TILE_DIM_K, FULL_TILE>(
...
template <int SF_TILE_DIM_M, int SF_TILE_DIM_K>
__global__ void __launch_bounds__(COL_COALESCED_THREADS)
    swizzle_col_scaling_direct_full_tile_kernel(const void* input, void* output, const int M,
                                                const int K, const int k_tiles_per_block,
                                                const int m_tiles_per_block) {
  swizzle_col_scaling_direct_full_tile_impl<SF_TILE_DIM_M, SF_TILE_DIM_K>(
      input, output, M, K, blockIdx.y, blockIdx.x, k_tiles_per_block, m_tiles_per_block);
}

template <int SF_TILE_DIM_M, int SF_TILE_DIM_K, int K_TILES_PER_TMA>
__device__ __forceinline__ void store_col_swizzle_tma_tile(const uint8_t* tile_u8, void* output,
                                                           const int m_tile, const int k_tile,
                                                           const int K) {
  static_assert(SF_TILE_DIM_M == COL_TMA_TILE_DIM && SF_TILE_DIM_K == 4,
                "TMA columnwise swizzle assumes 128-column full tiles.");
  constexpr int SF_TILE_SIZE = SF_TILE_DIM_M * SF_TILE_DIM_K;
...
template <int SF_TILE_DIM_M, int SF_TILE_DIM_K, int K_TILES_PER_TMA>
__device__ __forceinline__ void issue_col_swizzle_tma_copy(
    uint8_t* tile_u8, const void* input_map, uint64_t* mbar, const int stage, const int m_tile,
    const size_t k_row, const bool is_master_thread) {
  constexpr int TMA_TILE_BYTES = col_tma_tile_bytes<SF_TILE_DIM_K, K_TILES_PER_TMA>();
  copy_2d_to_shared(tile_u8 + stage * TMA_TILE_BYTES, input_map,
                    static_cast<size_t>(m_tile) * SF_TILE_DIM_M, k_row, TMA_TILE_BYTES,
                    &mbar[stage], is_master_thread);
}

template <int SF_TILE_DIM_M, int SF_TILE_DIM_K, int K_TILES_PER_TMA>
__global__ void __launch_bounds__(COL_COALESCED_THREADS)
    swizzle_col_scaling_tma_persistent_full_tile_kernel(
        const __grid_constant__ CUtensorMap input_map, void* output, const int M, const int K,
        const int total_blocks, const int num_k_blocks, const int k_tiles_per_block,
        const int m_tiles_per_block) {
#if (defined __CUDA_ARCH__) && (__CUDA_ARCH__ >= 1000)
  static_assert(SF_TILE_DIM_M == COL_TMA_TILE_DIM && SF_TILE_DIM_K == 4,
                "TMA columnwise swizzle assumes 128-column full tiles.");
  constexpr int THREADS_PER_BLOCK = COL_COALESCED_THREADS;
  constexpr int TMA_TILE_BYTES = col_tma_tile_bytes<SF_TILE_DIM_K, K_TILES_PER_TMA>();
...
#pragma nv_diag_suppress static_var_with_dynamic_init
  ...
  const int num_m_tiles = M / SF_TILE_DIM_M;
  const int total_tiles = num_m_tiles * num_k_blocks;
  (void)total_blocks;
  (void)m_tiles_per_block;

  const int first_tile = blockIdx.x;
  if (first_tile < total_tiles) {
    const int m_tile = first_tile / num_k_blocks;
    const int k_tile_block = first_tile - m_tile * num_k_blocks;
    const int first_k_tile = k_tile_block * k_tiles_per_block;
    issue_col_swizzle_tma_copy<SF_TILE_DIM_M, SF_TILE_DIM_K, K_TILES_PER_TMA>(
        tile_u8, reinterpret_cast<const void*>(&input_map), mbar, 0, m_tile,
        static_cast<size_t>(first_k_tile) * SF_TILE_DIM_K, is_master_thread);
  }

  for (int tile_id = first_tile, iter = 0; tile_id < total_tiles;
       tile_id += gridDim.x, ++iter) {
    const int stage = iter % COL_TMA_STAGES;
    const int parity = (iter / COL_TMA_STAGES) & 1;
    const int next_tile = tile_id + gridDim.x;
    if (next_tile < total_tiles) {
      ptx::cp_async_bulk_wait_group_read<1>();
      const int next_m_tile = next_tile / num_k_blocks;
      const int next_k_tile_block = next_tile - next_m_tile * num_k_blocks;
      const int next_first_k_tile = next_k_tile_block * k_tiles_per_block;
      issue_col_swizzle_tma_copy<SF_TILE_DIM_M, SF_TILE_DIM_K, K_TILES_PER_TMA>(
          tile_u8, reinterpret_cast<const void*>(&input_map), mbar,
          (iter + 1) % COL_TMA_STAGES, next_m_tile,
          static_cast<size_t>(next_first_k_tile) * SF_TILE_DIM_K, is_master_thread);
    }

    ptx::fence_proxy_async_shared_cta();
    ptx::mbarrier_wait_parity(&mbar[stage], parity);

    const int m_tile = tile_id / num_k_blocks;
    const int k_tile_block = tile_id - m_tile * num_k_blocks;
    const int first_k_tile = k_tile_block * k_tiles_per_block;
    store_col_swizzle_tma_tile<SF_TILE_DIM_M, SF_TILE_DIM_K, K_TILES_PER_TMA>(
        tile_u8 + stage * TMA_TILE_BYTES, output, m_tile, first_k_tile, K);
    __syncthreads();
    ptx::fence_proxy_async_shared_cta();
  }
...
#else
  NVTE_DEVICE_ERROR("swizzle_col_scaling_tma_persistent_full_tile_kernel requires SM 10.0+.");
#endif
}
...
__global__ void __launch_bounds__(COL_COALESCED_THREADS)
    grouped_swizzle_col_scaling_uniform_shape_tma_persistent_full_tile_kernel(
        const __grid_constant__ CUtensorMap input_map, void* output, const int M, const int K,
        const int total_blocks, const int blocks_per_tensor, const int num_k_blocks,
        const int k_tiles_per_block, const int m_tiles_per_block,
...
#pragma nv_diag_suppress static_var_with_dynamic_init
  ...
  const int num_m_tiles = M / SF_TILE_DIM_M;
  const int tiles_per_tensor = num_m_tiles * num_k_blocks;
  const int total_tiles = total_blocks * m_tiles_per_block;
  (void)blocks_per_tensor;

  const int first_tile = blockIdx.x;
  if (first_tile < total_tiles) {
    const int tensor_id = first_tile / tiles_per_tensor;
    const int tensor_tile_id = first_tile - tensor_id * tiles_per_tensor;
    const int m_tile = tensor_tile_id / num_k_blocks;
    const int k_tile_block = tensor_tile_id - m_tile * num_k_blocks;
    const int first_k_tile = k_tile_block * k_tiles_per_block;
    issue_col_swizzle_tma_copy<SF_TILE_DIM_M, SF_TILE_DIM_K, K_TILES_PER_TMA>(
        tile_u8, reinterpret_cast<const void*>(&input_map), mbar, 0, m_tile,
        static_cast<size_t>(tensor_id) * K +
            static_cast<size_t>(first_k_tile) * SF_TILE_DIM_K,
        is_master_thread);
  }
  ...
  for (int tile_id = first_tile, iter = 0; tile_id < total_tiles;
    ...
    if (next_tile < total_tiles) {
      ptx::cp_async_bulk_wait_group_read<1>();
      const int next_tensor_id = next_tile / tiles_per_tensor;
      const int next_tensor_tile_id = next_tile - next_tensor_id * tiles_per_tensor;
      const int next_m_tile = next_tensor_tile_id / num_k_blocks;
      const int next_k_tile_block = next_tensor_tile_id - next_m_tile * num_k_blocks;
      const int next_first_k_tile = next_k_tile_block * k_tiles_per_block;
      issue_col_swizzle_tma_copy<SF_TILE_DIM_M, SF_TILE_DIM_K, K_TILES_PER_TMA>(
          tile_u8, reinterpret_cast<const void*>(&input_map), mbar,
          (iter + 1) % COL_TMA_STAGES, next_m_tile,
          static_cast<size_t>(next_tensor_id) * K +
              static_cast<size_t>(next_first_k_tile) * SF_TILE_DIM_K,
          is_master_thread);
    }
    ...
    const int tensor_id = tile_id / tiles_per_tensor;
    const int tensor_tile_id = tile_id - tensor_id * tiles_per_tensor;
    const int m_tile = tensor_tile_id / num_k_blocks;
    const int k_tile_block = tensor_tile_id - m_tile * num_k_blocks;
    const int first_k_tile = k_tile_block * k_tiles_per_block;
    uint8_t* output_base =
        reinterpret_cast<uint8_t*>(output) + static_cast<size_t>(tensor_id) * output_stride_bytes;
    store_col_swizzle_tma_tile<SF_TILE_DIM_M, SF_TILE_DIM_K, K_TILES_PER_TMA>(
        tile_u8 + stage * TMA_TILE_BYTES, output_base, m_tile, first_k_tile, K);
    __syncthreads();
    ptx::fence_proxy_async_shared_cta();
  }
...
#else
  NVTE_DEVICE_ERROR(
      "grouped_swizzle_col_scaling_uniform_shape_tma_persistent_full_tile_kernel requires SM "
      "10.0+.");
#endif
}

template <int SF_TILE_DIM_M, int SF_TILE_DIM_K, bool FULL_TILE = false>
__device__ void swizzle_col_scaling_warp_tile_at_impl(const void* input, void* output, const int M,
                                                      const int K, const int original_M,
                                                      const int original_K, const int m_tile,
                                                      const int k_tile) {
  constexpr int SF_TILE_SIZE = SF_TILE_DIM_M * SF_TILE_DIM_K;
  const int lane = threadIdx.x & 31;
  const int m_base = m_tile * SF_TILE_DIM_M;
  ...
  uint4 input_segment_data;
  if constexpr (FULL_TILE) {
    input_segment_data =
        load_col_swizzle_segment_full_m(input_u8, M, m_base, k_base + input_row, input_segment);
  } else {
    input_segment_data = load_col_swizzle_segment(
...
__device__ void swizzle_col_scaling_warp_tile_impl(const void* input, void* output, const int M,
  ...
  swizzle_col_scaling_warp_tile_at_impl<SF_TILE_DIM_M, SF_TILE_DIM_K, FULL_TILE>(
      input, output, M, K, original_M, original_K, m_tile, k_tile);
}
...
template <int SF_TILE_DIM_M, int SF_TILE_DIM_K, bool FULL_TILE = false>
__device__ void swizzle_col_scaling_warp_tile_block_impl(
    const void* input, void* output, const int M, const int K, const int original_M,
    const int original_K, const int m_tile_block, const int k_tile_block,
    const int k_tiles_per_block, const int m_tiles_per_block) {
  const int num_tiles_m = M / SF_TILE_DIM_M;
  ...
  for (int local_m_tile = 0; local_m_tile < active_m_tiles; ++local_m_tile) {
    const int m_tile = first_m_tile + local_m_tile;
    for (int k_tile_offset = warp_id; k_tile_offset < active_k_tiles;
         k_tile_offset += COL_WARP_TILE_WARPS) {
      swizzle_col_scaling_warp_tile_at_impl<SF_TILE_DIM_M, SF_TILE_DIM_K, FULL_TILE>(
          input, output, M, K, original_M, original_K, m_tile, first_k_tile + k_tile_offset);
    }
  }
}
...
template <int SF_TILE_DIM_M, int SF_TILE_DIM_K, bool FULL_TILE = false>
__global__ void __launch_bounds__(COL_WARP_TILE_THREADS)
    swizzle_col_scaling_warp_tile_block_kernel(const void* input, void* output, const int M,
                                               const int K, const int original_M,
                                               const int original_K,
                                               const int k_tiles_per_block,
                                               const int m_tiles_per_block) {
  swizzle_col_scaling_warp_tile_block_impl<SF_TILE_DIM_M, SF_TILE_DIM_K, FULL_TILE>(
      input, output, M, K, original_M, original_K, blockIdx.y, blockIdx.x, k_tiles_per_block,
      m_tiles_per_block);
}
...
struct ColSwizzleTmaMapCacheEntry {
  void* ptr;
  uint64_t global_y;
  uint64_t global_x;
  uint32_t shmem_y;
...
void get_col_swizzle_tma_input_map(CUtensorMap& input_map, void* input_ptr,
                                   const uint64_t global_y, const uint64_t global_x,
                                   const uint32_t shmem_y) {
  static std::mutex mutex;
  static std::vector<ColSwizzleTmaMapCacheEntry> cache;

  std::lock_guard<std::mutex> lock(mutex);
  for (const auto& entry : cache) {
    if (entry.ptr == input_ptr && entry.global_y == global_y && entry.global_x == global_x &&
        entry.shmem_y == shmem_y) {
      input_map = entry.map;
      return;
    }
  }

  alignas(64) CUtensorMap new_map{};
  SimpleTensor input_map_tensor(input_ptr, {static_cast<size_t>(global_y),
                                            static_cast<size_t>(global_x)},
                                DType::kByte);
  create_2D_tensor_map(new_map, input_map_tensor, global_y, global_x, shmem_y,
                       COL_TMA_TILE_DIM, static_cast<uint32_t>(global_x), 0, 8,
                       CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_128B_ATOM_32B);
  cache.push_back({input_ptr, global_y, global_x, shmem_y, new_map});
  input_map = new_map;
}

bool use_small_swizzle_parallelism(const size_t output_scale_numel, const size_t scale_elem_size) {
  return output_scale_numel * scale_elem_size <= SMALL_SWIZZLE_OUTPUT_BYTES;
}

int select_col_coalesced_k_tiles_per_block(const int num_tiles_k,
                                           const bool prefer_small_parallelism) {
  // Small cases and short per-tensor K ranges keep the 128-row tile to preserve
  // launch parallelism. Long K ranges use a 256-row TMA tile to amortize TMA
  // barriers and grouped-variable metadata lookup.
  if (prefer_small_parallelism || num_tiles_k < COL_TMA_K_TILES_LARGE ||
      num_tiles_k % COL_TMA_K_TILES_LARGE != 0) {
    return COL_COALESCED_K_TILES_PER_BLOCK;
  }
  return COL_TMA_K_TILES_LARGE;
}

bool use_col_tma_k_tiles_per_block(const int k_tiles_per_block) {
  return k_tiles_per_block == COL_TMA_K_TILES_SMALL ||
         k_tiles_per_block == COL_TMA_K_TILES_LARGE;
}
...
int col_persistent_grid_blocks(const int total_blocks, const int num_sms = cuda::sm_count()) {
  return std::min(total_blocks, num_sms * COL_PERSISTENT_TARGET_BLOCKS_PER_SM);
}

template <typename Kernel>
int col_tma_persistent_grid_blocks(const int total_blocks, Kernel kernel_fn,
                                   const int dynamic_smem_size,
                                   const int num_sms = cuda::sm_count()) {
  ...
  static std::vector<std::vector<CacheEntry>> cache(cuda::num_devices());
  static std::mutex mutex;
  NVTE_CHECK(0 <= device_id && device_id < cuda::num_devices(), "invalid CUDA device ID");

  {
    std::lock_guard<std::mutex> lock(mutex);
    for (const auto& entry : cache[device_id]) {
      if (entry.kernel == kernel_fn && entry.dynamic_smem_size == dynamic_smem_size) {
        return std::min(total_blocks, num_sms * entry.target_blocks_per_sm);
      }
    }
  }
  ...
  int target_blocks_per_sm;
  {
    NVTE_CHECK_CUDA(cudaFuncSetAttribute(
        kernel_fn, cudaFuncAttributeMaxDynamicSharedMemorySize, dynamic_smem_size));
    int max_active_blocks_per_sm;
    NVTE_CHECK_CUDA(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &max_active_blocks_per_sm, kernel_fn, COL_COALESCED_THREADS, dynamic_smem_size));
    NVTE_CHECK(max_active_blocks_per_sm > 0, "Occupancy query returned 0 blocks per SM.");
    target_blocks_per_sm = std::min(COL_PERSISTENT_TARGET_BLOCKS_PER_SM,
                                    max_active_blocks_per_sm);
  }

  {
    std::lock_guard<std::mutex> lock(mutex);
    cache[device_id].push_back({kernel_fn, dynamic_smem_size, target_blocks_per_sm});
  }
  return std::min(total_blocks, num_sms * target_blocks_per_sm);
}
...
__global__ void __launch_bounds__(ROW_COALESCED_THREADS)
    grouped_swizzle_row_scaling_uniform_shape_coalesced_full_m_kernel(
        const void* input, void* output, const int K, const size_t input_stride_bytes,
        const size_t output_stride_bytes) {
  const int tensor_id = blockIdx.y;
  const uint8_t* input_base =
      reinterpret_cast<const uint8_t*>(input) + tensor_id * input_stride_bytes;
  ...
  const int first_m_tile = blockIdx.x * M_TILES_PER_BLOCK;
  swizzle_row_scaling_coalesced_full_m_impl<SF_TILE_DIM_M, SF_TILE_DIM_K, M_TILES_PER_BLOCK>(
      input_base, output_base, K, first_m_tile, slm_v4i);
}
...
void launch_swizzle_row_scaling_coalesced_full_m(const void* input, void* output, const int M,
  ...
  set_dynamic_smem_if_needed(
      swizzle_row_scaling_coalesced_full_m_kernel<SF_TILE_DIM_M, SF_TILE_DIM_K,
                                                  M_TILES_PER_BLOCK>,
      slm_size, cached_smem_size);
  swizzle_row_scaling_coalesced_full_m_kernel<SF_TILE_DIM_M, SF_TILE_DIM_K, M_TILES_PER_BLOCK>
      <<<DIVUP(num_tiles_m, M_TILES_PER_BLOCK), ROW_COALESCED_THREADS, slm_size, stream>>>(
          input, output, K);
}

template <int SF_TILE_DIM_M, int SF_TILE_DIM_K, int M_TILES_PER_BLOCK>
void launch_grouped_swizzle_row_scaling_uniform_shape_coalesced_full_m(
    const void* input, void* output, const int M, const int K, const size_t input_stride_bytes,
    const size_t output_stride_bytes, const int num_tensors, cudaStream_t stream) {
  const int num_tiles_m = M / SF_TILE_DIM_M;
  const int slm_size =
      row_coalesced_slm_size_bytes<SF_TILE_DIM_M, SF_TILE_DIM_K>(K, sizeof(uint8_t)) *
      M_TILES_PER_BLOCK;
  ...
  grouped_swizzle_row_scaling_uniform_shape_coalesced_full_m_kernel<
      SF_TILE_DIM_M, SF_TILE_DIM_K, M_TILES_PER_BLOCK>
      <<<dim3(DIVUP(num_tiles_m, M_TILES_PER_BLOCK), num_tensors), ROW_COALESCED_THREADS,
         slm_size, stream>>>(input, output, K, input_stride_bytes, output_stride_bytes);
}
...
template <int SF_TILE_DIM_M, int SF_TILE_DIM_K, bool FULL_TILE = false>
__global__ void __launch_bounds__(COL_WARP_TILE_THREADS)
    grouped_swizzle_col_scaling_uniform_shape_warp_tile_block_kernel(
        const void* input, void* output, const int M, const int K, const int original_M,
        const int original_K, const size_t input_stride_bytes, const size_t output_stride_bytes,
        const int k_tiles_per_block, const int m_tiles_per_block) {
  const int tensor_id = blockIdx.z;
...
__global__ void __launch_bounds__(COL_COALESCED_THREADS)
  ...
  swizzle_col_scaling_coalesced_batched_m_impl<SF_TILE_DIM_M, SF_TILE_DIM_K, FULL_TILE>(
      input_base, output_base, M, K, original_M, original_K, blockIdx.y, blockIdx.x,
      k_tiles_per_block, m_tiles_per_block, slm_i32);
}

template <int SF_TILE_DIM_M, int SF_TILE_DIM_K>
__global__ void __launch_bounds__(COL_COALESCED_THREADS)
    grouped_swizzle_col_scaling_uniform_shape_direct_full_tile_kernel(
  ...
  uint8_t* output_base = reinterpret_cast<uint8_t*>(output) + tensor_id * output_stride_bytes;
  swizzle_col_scaling_direct_full_tile_impl<SF_TILE_DIM_M, SF_TILE_DIM_K>(
      input_base, output_base, M, K, blockIdx.y, blockIdx.x, k_tiles_per_block,
      m_tiles_per_block);
}
...
void swizzle_scaling_factors(const Tensor* input, Tensor* output, cudaStream_t stream) {
    ...
    if (use_row_coalesced) {
      ...
      if (use_full_m_fast_path && m_tiles_per_block == 8) {
        ...
        launch_swizzle_row_scaling_coalesced_full_m<SF_TILE_DIM_M, SF_TILE_DIM_K, 5>(
            input_scale_inv_ptr, output_scale_inv_ptr, m, k, stream);
      } else if (use_full_m_fast_path && m_tiles_per_block == 1) {
        launch_swizzle_row_scaling_coalesced_full_m<SF_TILE_DIM_M, SF_TILE_DIM_K, 1>(
            input_scale_inv_ptr, output_scale_inv_ptr, m, k, stream);
      } else {
  ...
  if (columnwise_swizzle) {
    ...
    if (use_blackwell_col_coalesced_swizzle(sizeof(uint8_t))) {
      const bool prefer_small_col_parallelism = use_small_swizzle_parallelism(
          output->columnwise_scale_inv.numel(), typeToSize(output->columnwise_scale_inv.dtype));
      const int k_tiles_per_block =
          select_col_coalesced_k_tiles_per_block(num_tiles_k, prefer_small_col_parallelism);
      const int m_tiles_per_block =
      ...
      const dim3 num_blocks(DIVUP(num_tiles_k, k_tiles_per_block),
                            DIVUP(num_tiles_m, m_tiles_per_block));
      const bool use_full_tile_fast_path =
          original_M == m && original_K == k && num_tiles_k % k_tiles_per_block == 0 &&
          num_tiles_m % m_tiles_per_block == 0;
      const bool use_tma_full_tile =
          use_full_tile_fast_path && !prefer_small_col_parallelism &&
          use_col_tma_k_tiles_per_block(k_tiles_per_block);
      if (use_tma_full_tile) {
        ...
        get_col_swizzle_tma_input_map(input_map, input->columnwise_scale_inv.dptr,
        ...
        if (k_tiles_per_block == COL_TMA_K_TILES_LARGE) {
          ...
          swizzle_col_scaling_tma_persistent_full_tile_kernel<SF_TILE_DIM_M, SF_TILE_DIM_K,
              ...
              <<<persistent_blocks, COL_COALESCED_THREADS, tma_smem_size, stream>>>(
                  input_map, output->columnwise_scale_inv.dptr, m, k, total_blocks, num_k_blocks,
                  k_tiles_per_block, m_tiles_per_block);
        } else {
      ...
      } else if (use_full_tile_fast_path) {
        swizzle_col_scaling_direct_full_tile_kernel<SF_TILE_DIM_M, SF_TILE_DIM_K>
            <<<num_blocks, COL_COALESCED_THREADS, 0, stream>>>(
        ...
        swizzle_col_scaling_coalesced_kernel<SF_TILE_DIM_M, SF_TILE_DIM_K>
            <<<num_blocks, COL_COALESCED_THREADS, slm_size, stream>>>(
                input->columnwise_scale_inv.dptr, output->columnwise_scale_inv.dptr, m, k,
                original_M, original_K, k_tiles_per_block, m_tiles_per_block);
      }
...
template <int SF_TILE_DIM_M, int SF_TILE_DIM_K>
__global__ void __launch_bounds__(COL_WARP_TILE_THREADS)
    grouped_swizzle_col_scaling_variable_shape_warp_tile_block_kernel(
        const void* input, void* output, const int64_t* k_array, int num_tensors,
  ...
  swizzle_col_scaling_warp_tile_block_impl<SF_TILE_DIM_M, SF_TILE_DIM_K>(
      input_base, output_base, padded_m, padded_k, original_M, original_K, m_tile_block,
...
__global__ void __launch_bounds__(COL_COALESCED_THREADS)
    grouped_swizzle_col_scaling_variable_shape_direct_full_tile_kernel(
        const void* input, void* output, const int64_t* k_array, int num_tensors,
        size_t scale_elem_size, size_t common_m, const int k_tiles_per_block,
        const int m_tiles_per_block, const int total_blocks) {
  const int padded_m = static_cast<int>(round_up_to_multiple(common_m, SF_TILE_DIM_M));
  const int num_tiles_m = padded_m / SF_TILE_DIM_M;
  const int num_m_blocks = DIVUP(num_tiles_m, m_tiles_per_block);
...
__global__ void __launch_bounds__(COL_COALESCED_THREADS)
    grouped_swizzle_col_scaling_variable_shape_tma_persistent_full_tile_kernel(
        const __grid_constant__ CUtensorMap input_map, const void* input, void* output,
        const int64_t* k_array, int num_tensors, size_t scale_elem_size, size_t common_m,
        const int k_tiles_per_block, const int m_tiles_per_block, const int total_blocks) {
#if (defined __CUDA_ARCH__) && (__CUDA_ARCH__ >= 1000)
  static_assert(SF_TILE_DIM_M == COL_TMA_TILE_DIM && SF_TILE_DIM_K == 4,
                "TMA columnwise swizzle assumes 128-column full tiles.");
  constexpr int THREADS_PER_BLOCK = COL_COALESCED_THREADS;
  constexpr int TMA_TILE_BYTES = col_tma_tile_bytes<SF_TILE_DIM_K, K_TILES_PER_TMA>();
  if (k_tiles_per_block != K_TILES_PER_TMA || scale_elem_size != sizeof(uint8_t)) return;
...
#pragma nv_diag_suppress static_var_with_dynamic_init
  ...
  if (current.valid && current.full_k_tile) {
    copy_2d_to_shared(tile_u8, reinterpret_cast<const void*>(&input_map),
                      ...
                      TMA_TILE_BYTES, &mbar[0], is_master_thread);
  }
  ...
  for (int linear_tile_id = first_tile, iter = 0; linear_tile_id < total_tiles;
    ...
    if (next.valid && next.full_k_tile) {
      ...
      ptx::cp_async_bulk_wait_group_read<1>();
      copy_2d_to_shared(tile_u8 + next_stage * TMA_TILE_BYTES,
                        ...
                        TMA_TILE_BYTES, &mbar[next_stage], is_master_thread);
    }
...
__global__ void __launch_bounds__(COL_COALESCED_THREADS)
  ...
  swizzle_col_scaling_coalesced_batched_m_impl<SF_TILE_DIM_M, SF_TILE_DIM_K>(
      input_base, output_base, padded_m, padded_k, original_M, original_K, m_tile_block,
      k_tile_block, k_tiles_per_block, m_tiles_per_block, slm_i32);
}
...
void swizzle_grouped_scaling_factors(const GroupedTensor* input, GroupedTensor* output,
  ...
  if (!is_variable_shape) {
    ...
    auto launch_grouped_swizzle = [&](bool rowwise) {
      ...
      const bool prefer_small_parallelism =
          use_small_swizzle_parallelism(output_scale_numel, scale_elem_size);
      ...
      const int col_coalesced_k_tiles_per_block =
          select_col_coalesced_k_tiles_per_block(num_tiles_k, prefer_small_parallelism);
      ...
      if (use_row_coalesced) {
        ...
        num_blocks = dim3(DIVUP(num_tiles_k, col_coalesced_k_tiles_per_block),
                          DIVUP(num_tiles_m, col_coalesced_m_tiles), input->num_tensors);
      ...
      if (use_row_coalesced) {
        const bool use_full_m_fast_path =
            original_M == static_cast<int>(padded_m) && num_tiles_m % row_coalesced_m_tiles == 0;
        if (use_full_m_fast_path && row_coalesced_m_tiles == 8) {
          launch_grouped_swizzle_row_scaling_uniform_shape_coalesced_full_m<
              SF_TILE_DIM_M, SF_TILE_DIM_K, 8>(
          ...
          launch_grouped_swizzle_row_scaling_uniform_shape_coalesced_full_m<
              ...
              input_stride_bytes, output_stride_bytes, static_cast<int>(input->num_tensors),
              stream);
        } else if (use_full_m_fast_path && row_coalesced_m_tiles == 1) {
          launch_grouped_swizzle_row_scaling_uniform_shape_coalesced_full_m<
              SF_TILE_DIM_M, SF_TILE_DIM_K, 1>(
              input_ptr, output_ptr, static_cast<int>(padded_m), static_cast<int>(padded_k),
              input_stride_bytes, output_stride_bytes, static_cast<int>(input->num_tensors),
              stream);
        } else {
        ...
        const bool use_full_tile_fast_path =
            original_M == static_cast<int>(padded_m) && original_K == static_cast<int>(padded_k) &&
            num_tiles_k % col_coalesced_k_tiles_per_block == 0 &&
            num_tiles_m % col_coalesced_m_tiles == 0;
        const bool use_tma_full_tile =
            use_full_tile_fast_path && !prefer_small_parallelism &&
            use_col_tma_k_tiles_per_block(col_coalesced_k_tiles_per_block) &&
            scale_elem_size == sizeof(uint8_t) &&
            input_stride_bytes == padded_scale_elems * scale_elem_size;
        if (use_tma_full_tile) {
          alignas(64) CUtensorMap input_map{};
          get_col_swizzle_tma_input_map(
              input_map, const_cast<void*>(input_ptr),
              static_cast<uint64_t>(input->num_tensors) * padded_k,
              static_cast<uint64_t>(padded_m),
              static_cast<uint32_t>(col_coalesced_k_tiles_per_block * SF_TILE_DIM_K));
          const int num_k_blocks = DIVUP(num_tiles_k, col_coalesced_k_tiles_per_block);
          ...
          if (col_coalesced_k_tiles_per_block == COL_TMA_K_TILES_LARGE) {
            ...
            const int persistent_blocks = col_tma_persistent_grid_blocks(
                total_blocks,
                grouped_swizzle_col_scaling_uniform_shape_tma_persistent_full_tile_kernel<
                    SF_TILE_DIM_M, SF_TILE_DIM_K, K_TILES_PER_TMA>,
                tma_smem_size);
            grouped_swizzle_col_scaling_uniform_shape_tma_persistent_full_tile_kernel<
                SF_TILE_DIM_M, SF_TILE_DIM_K, K_TILES_PER_TMA>
                <<<persistent_blocks, COL_COALESCED_THREADS, tma_smem_size, stream>>>(
                    input_map, output_ptr, static_cast<int>(padded_m), static_cast<int>(padded_k),
                    total_blocks, blocks_per_tensor, num_k_blocks, col_coalesced_k_tiles_per_block,
          ...
          grouped_swizzle_col_scaling_uniform_shape_direct_full_tile_kernel<SF_TILE_DIM_M,
                                                                            SF_TILE_DIM_K>
              <<<num_blocks, COL_COALESCED_THREADS, 0, stream>>>(
                  input_ptr, output_ptr, padded_m, padded_k, input_stride_bytes,
                  output_stride_bytes, col_coalesced_k_tiles_per_block, col_coalesced_m_tiles);
        } else {
    ...
    if (has_rowwise_scale_inv && input->all_same_last_dim() && input->first_dims.has_data()) {
      ...
      const int original_k = static_cast<int>(DIVUP(k, static_cast<size_t>(MXFP8_BLOCK_SIZE)));
      const size_t stacked_m = input->logical_shape.data[0];
      const size_t stacked_scale_elems = stacked_m * padded_k;
      // If every variable-M tensor already starts and ends on an M tile, the
      // rowwise scale buffer is equivalent to one stacked compact matrix.
      const bool can_use_stacked_rowwise =
          stacked_m % SF_TILE_DIM_M == 0 && input->scale_inv.numel() == stacked_scale_elems &&
          output->scale_inv.numel() == stacked_scale_elems;
      if (use_blackwell_row_coalesced_swizzle<SF_TILE_DIM_M, SF_TILE_DIM_K>(
        ...
        if (can_use_stacked_rowwise) {
          const int stacked_num_tiles_m = static_cast<int>(stacked_m / SF_TILE_DIM_M);
          ...
          if (use_full_m_fast_path && m_tiles_per_block == 8) {
            ...
            static int cached_variable_row_stacked_coalesced = -1;
            set_dynamic_smem_if_needed(
                swizzle_row_scaling_coalesced_batched_m_kernel<SF_TILE_DIM_M, SF_TILE_DIM_K>,
                dynamic_smem_size, cached_variable_row_stacked_coalesced);
            swizzle_row_scaling_coalesced_batched_m_kernel<SF_TILE_DIM_M, SF_TILE_DIM_K>
                <<<DIVUP(stacked_num_tiles_m, m_tiles_per_block), ROW_COALESCED_THREADS,
                   dynamic_smem_size, stream>>>(
                    input->scale_inv.dptr, output->scale_inv.dptr, static_cast<int>(stacked_m),
                    static_cast<int>(padded_k), static_cast<int>(stacked_m), m_tiles_per_block);
          }
        } else {
          const size_t total_m_tiles_upper =
              DIVUP(stacked_m, static_cast<size_t>(SF_TILE_DIM_M)) + num_tensors;
    ...
    if (has_columnwise_scale_inv && input->all_same_last_dim() && input->first_dims.has_data()) {
      ...
      if (use_blackwell_col_coalesced_swizzle(scale_elem_size)) {
        ...
        const size_t stacked_k_scales =
            DIVUP(input->logical_shape.data[0], static_cast<size_t>(MXFP8_BLOCK_SIZE));
        const int avg_tiles_k = static_cast<int>(
            DIVUP(DIVUP(stacked_k_scales, static_cast<size_t>(SF_TILE_DIM_K)), num_tensors));
        const int k_tiles_per_block =
            select_col_coalesced_k_tiles_per_block(avg_tiles_k, prefer_small_col_parallelism);
        const int m_tiles_per_block =
            col_coalesced_m_tiles_per_block(num_tiles_m, prefer_small_col_parallelism);
        const int slm_size =
            col_coalesced_slm_size_bytes<SF_TILE_DIM_M, SF_TILE_DIM_K>(k_tiles_per_block);
        ...
        const size_t num_m_blocks =
            DIVUP(static_cast<size_t>(num_tiles_m), static_cast<size_t>(m_tiles_per_block));
        const size_t num_blocks = num_m_blocks * k_block_upper;
        const bool can_use_direct_full_tile =
            m == padded_m && input->logical_shape.data[0] % MXFP8_BLOCK_SIZE == 0 &&
            input->columnwise_scale_inv.numel() ==
                (input->logical_shape.data[0] / MXFP8_BLOCK_SIZE) * padded_m &&
            output->columnwise_scale_inv.numel() ==
                (input->logical_shape.data[0] / MXFP8_BLOCK_SIZE) * padded_m;
        const bool can_use_tma_full_tile =
            can_use_direct_full_tile && use_col_tma_k_tiles_per_block(k_tiles_per_block) &&
            scale_elem_size == sizeof(uint8_t);
        if (can_use_tma_full_tile) {
          ...
          if (k_tiles_per_block == COL_TMA_K_TILES_LARGE) {
            ...
            const int persistent_blocks = col_tma_persistent_grid_blocks(
                ...
                grouped_swizzle_col_scaling_variable_shape_tma_persistent_full_tile_kernel<
                    SF_TILE_DIM_M, SF_TILE_DIM_K, K_TILES_PER_TMA>,
                tma_smem_size, num_SMs);
            grouped_swizzle_col_scaling_variable_shape_tma_persistent_full_tile_kernel<
                SF_TILE_DIM_M, SF_TILE_DIM_K, K_TILES_PER_TMA>
                <<<persistent_blocks, COL_COALESCED_THREADS, tma_smem_size, stream>>>(
                    input_map, input->columnwise_scale_inv.dptr,
                    output->columnwise_scale_inv.dptr, m_array, static_cast<int>(num_tensors),
        ...
        } else if (can_use_direct_full_tile) {
          const int total_blocks = static_cast<int>(num_blocks);
          ...
          grouped_swizzle_col_scaling_variable_shape_direct_full_tile_kernel<SF_TILE_DIM_M,
              ...
              <<<persistent_blocks, COL_COALESCED_THREADS, 0, stream>>>(
                  ...
                  m_tiles_per_block, total_blocks);
        } else {
