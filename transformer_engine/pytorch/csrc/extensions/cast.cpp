...
py::object group_quantize(const at::Tensor &tensor, py::handle quantizer, const size_t num_tensors,
  ...
  if (detail::IsMXFP8Quantizers(quantizer.ptr())) {
    ...
    grouped_quantization_mode =
        GroupedQuantizationMode::FP8_CURRENT_SCALING_GROUPED_QUANTIZE;
  }
  ...
  switch (grouped_quantization_mode) {
    ...
    default:
      NVTE_ERROR("group_quantize: only supports FP8 current scaling, NVFP4, or MXFP8 quantizer.");
      break;
  }
