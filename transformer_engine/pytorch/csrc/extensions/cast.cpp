...
py::object group_quantize(const at::Tensor &tensor, py::handle quantizer, const size_t num_tensors,
  ...
  switch (grouped_quantization_mode) {
    case GroupedQuantizationMode::FP8_CURRENT_SCALING_GROUPED_QUANTIZE: {
      auto *fp8_current_scaling_quantizer_cpp =
          dynamic_cast<Float8CurrentScalingQuantizer *>(quantizer_cpp.get());
      NVTE_CHECK(fp8_current_scaling_quantizer_cpp != nullptr,
                 "group_quantize: invalid Float8CurrentScalingQuantizer.");
      NVTE_CHECK(!fp8_current_scaling_quantizer_cpp->with_amax_reduction,
                 "group_quantize with Float8CurrentScalingQuantizer does not support "
                 "with_amax_reduction=True.");
      QuantizationConfigWrapper quant_config_cpp;
      quant_config_cpp.set_force_pow_2_scales(
          fp8_current_scaling_quantizer_cpp->force_pow_2_scales);
      quant_config_cpp.set_amax_epsilon(fp8_current_scaling_quantizer_cpp->amax_epsilon);
      NVTE_SCOPED_GIL_RELEASE({
    ...
    default:
      NVTE_ERROR(
          "group_quantize: only support Float8CurrentScalingQuantizer, NVFP4, or MXFP8 "
          "quantizer.");
      break;
  }
